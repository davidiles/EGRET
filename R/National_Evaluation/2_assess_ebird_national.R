...
#
# Identical filters to assess_BAM_national.R (thresholds from config.R).
# The date-window filter is applied per species inside the loop (Section 9).
# ------------------------------------------------------------------------------
...
stats_path <- file.path(
  OUT_NATIONAL_EBIRD_STATS, "eBird_preds_vs_BAM_data.rds"
)
...
for (spcd in species_vec) {
  ...
  # -- 10c. Match to eBird species code ---------------------------------------
  # Name matching uses common_name from ebirdst_runs.  Where a common name
  # maps to multiple eBird codes (e.g. due to taxonomic splits), the first
  # match is used unless a known ambiguity override applies.
  spcd_ebird_matches <- ebirdst_runs %>%
    dplyr::filter(common_name == sp_english) %>%
    dplyr::pull(species_code)

  if (length(spcd_ebird_matches) == 0) {
    message(sp_english, " not available in eBird; skipping")
    next
  }

  # Known ambiguity: Yellow-bellied Sapsucker shares a common name between
  # nominate and subspecies entries; use the second match (nominate).
  spcd_ebird <- if (sp_english == "Yellow-bellied Sapsucker" &&
                    length(spcd_ebird_matches) >= 2) {
    spcd_ebird_matches[2]
  } else {
    spcd_ebird_matches[1]
  }
  ...
  pdf_path <- file.path(
    ...
    paste0(spcd, "_eBird_national.pdf")
  )
  ...
  hex_summary_path <- file.path(
    OUT_HEX_SUMMARIES,
    paste0(spcd, "_eBird_hex_summary.rds")
  )
