...
plot_output_dir <- paste0(category_output_dir,"category_plots/")
...
# ── Plot one category for one species ────────────────────────────────────────
...
plot_category_comparison <- function(hex_compare,
  ...
  ggplot() +
    geom_sf(data = region_national, fill = "gray95") +
    geom_sf(
...
process_species <- function(spcd_bam,
  ...
  combined_plot <- wrap_plots(
    ...
    plot_annotation(title = spcd_bam)
  
  ggsave(
    filename = file.path(plot_output_dir, paste0(spcd_bam, "_category_comparison.png")),
    plot = combined_plot,
    width = 14,
    height = 7,
    dpi = 300
...
write_csv(
  comparison_stats_all,
  file.path(plot_output_dir, "category_comparison_summary_stats.csv")
)
...
combined <- combined %>%
  left_join(pred_totals, by = "spcd_bam") %>%
  mutate(
    log_pred_sum_BAM = log10(pred_sum_BAM + 1),
    log_pred_sum_eBird = log10(pred_sum_eBird + 1),
    total_flags_bam = n_absent_bam + n_highpred_bam,
    total_flags_ebird = n_absent_ebird + n_highpred_ebird
  )
